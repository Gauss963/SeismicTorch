c**********************************************************************
c  F-K:	@(#) fk.f			3.4 11/04/2019
c
c  Copyright (c) 1999-2022 by L. Zhu
...
c	11/04/2019  Lupei Zhu   Replace formatted read of (x t0 fout) by list-directed.
c******************************************************************************
...
c input distance ranges
	 ...
	 call exit(1)
      ...
      tmax = 0.
      do ix=1,nx
	write(0, *) 'Input x t0 output_name'
        read(*,*)x(ix),t0(ix),fout(ix)
        write(0,*)x(ix),t0(ix),fout(ix)
...
c*************** do inverse fourier transform
      ...
      do ix=1,nx
	 if ( nfft2.EQ.1 ) then
	    write(*,'(f8.3,9e11.3)')x(ix),(real(sum(ix,l,1)),l=1,nCom)
	 else
