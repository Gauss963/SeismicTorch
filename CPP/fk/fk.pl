#!/usr/bin/perl
#
# perl code for computing Green's functions using FK or sFK.
#
# 	Lupei Zhu	5/3/96		Seismo Lab, Caltech

# Rev. History
#	Lupei Zhu	11/05/2019	Add sFK
#	Lupei Zhu	12/23/2019	apply EFT to focal depth only.
...
my $halfS = 0.5;	# sFK's half source layer thickness
...
-M: model name and source depth in km.
    ...
    f triggers earth flattening to depth (the model should be transformed already) (off).
    k indicates that the 3rd column is vp/vs ratio (vp).
...
-N: nt is the number of points, must be 2^n ($nft).
    ...
    dk<0.5 is the non-dimensional sampling interval of wavenumber. sfk will be used if dk>1. ($dk).
...
if ($dk>1) {	# sfk needs the source in a layer, not on an interface
  if ($th[$src_layer]<$halfS || $th[$src_layer-1]<$halfS) {
     printf STDERR "Error **** the source layer thickness is too small for sFK\n";
     exit(0);
  }
  $th[$src_layer-1] -= $halfS;
  insert_intf($s_depth+$halfS);
  $num_layer++;
  $rcv_layer++ unless $rcv_layer<$src_layer;
}
...
printf REFL "%5d\n",$#dist+1;
foreach (@dist) {
    printf REFL "%f %f '$dirnm/$_$rdep.$fk.'\n",$_*$deg2km,$t0{$_};
}
close(REFL);
...
foreach (@dist) {
  system("sachd $sac_com{$_} f $dirnm/$_$rdep.$fk.[0,5,a]");
}
