...
******************************************************************/
void nmtensor(float iso, float clvd, float str, float dip, float rake, float vti[5], float tensor[3][3]) {
  ...
  if (vti[0]>0.) {
     ...
     ptt[4] = 2*tensor[0][2];
     ptt[5] = 2*tensor[0][1];
     potency2moment(vti, ptt, mtt);
     // the norm of the moment tensor is sqrt(2)
...
/****************************************************************
  compute moment tensor mtt from potency tensor ptt in VTI medium
  mtt_i = C_ij*ptt_j, 1=x, 2=y, 3=z.
  vti = (C44, C33, C11, C66, C13)
****************************************************************/
void potency2moment(float vti[5],float ptt[6], float mtt[6]) {
  float c12 = vti[2]-2*vti[3];	// C12 = C11-2*C66
  mtt[0] = vti[2]*ptt[0] + c12*ptt[1]+vti[4]*ptt[2];
  mtt[1] = vti[2]*ptt[1] + c12*ptt[0]+vti[4]*ptt[2];
  mtt[2] = vti[1]*ptt[2] + vti[4]*(ptt[0]+ptt[1]);
  mtt[3] = vti[0]*ptt[3];
  mtt[4] = vti[0]*ptt[4];
  mtt[5] = vti[3]*ptt[5];
  return;
