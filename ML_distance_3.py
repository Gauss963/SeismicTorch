...
for datadir in RCEC:
    ...
    floor_name = datadir
    floor_number = RCEC.index(floor_name) + 1
    ...
    X = np.column_stack((distance_array, magnitude_array, depth_array))
    ...
    # 显示图形
    plt.title('Scatter Plot with Depth and SVM Separation, ' + floor_name + ' (RCEC ' + floor_number + 'f)' )
    plt.grid(True)
    plt.savefig('./Dist_ML_Plot/plot_with_SVM_separation_RCEC_' + floor_number + 'f_' + floor_name + '.pdf' , dpi = 900, bbox_inches = 'tight')
...
for datadir in IES:
    ...
    depth_array = np.array([])
    floor_name = datadir
    floor_number = IES.index(floor_name) + 1
    floor_number = str(floor_number)
    ...
    # 显示图形
    plt.title('Scatter Plot with Depth and SVM Separation, ' + floor_name + ' (IES ' + floor_number + 'f)' )
    plt.grid(True)
    plt.savefig('./Dist_ML_Plot/plot_with_SVM_separation_IES_' + floor_number + 'f_' + floor_name + '.pdf' , dpi = 900, bbox_inches = 'tight')